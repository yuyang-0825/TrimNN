...
train_config = {
    ...
    "model" : "RGCN", # CNN, RNN, TXL, RGCN, RGIN, RSIN
    ...
    if all([os.path.exists(os.path.join(train_config["save_data_dir"],
        ...
        for data_type in data_loaders:
            if train_config["model"] in ["RGCN", "RGIN"]:
                ...
                dataset_new =GraphAdjDataset(list())
                dataset_new.data = dataset.data + dataset.data
                # dataset_new = ConcatDataset([dataset.data, dataset.data])
                sampler = Sampler(dataset_new, group_by=["graph", "pattern"], batch_size=train_config["batch_size"], shuffle=data_type=="train", drop_last=False)
                data_loader = DataLoader(dataset_new,
