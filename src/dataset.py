...
class GraphAdjDataset(data.Dataset):
    ...
    def preprocess(x):
        ...
        pattern_dglgraph.edata["label"] = torch.from_numpy(np.array(pattern.es["label"], dtype=np.int64))
        ...
        graph_dglgraph.edata["label"] = torch.from_numpy(np.array(graph.es["label"], dtype=np.int64))
