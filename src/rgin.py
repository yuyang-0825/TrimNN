...
class RGIN(GraphAdjModel):
    def __init__(self, config):
        ...
        self.distance_PE = config["distance_PE"]
        self.PE_alpha = config["PE_alpha"]
    ...
    def forward(self, pattern, pattern_len, graph, graph_len):
        ...
        if self.distance_PE:
            # print("use PE")
            # pattern_PE_embedding = self.PE_linear(pattern.ndata['distance_PE'].unsqueeze(-1).to(torch.float))
            # pattern_output = torch.cat((pattern_emb, pattern_PE_embedding), dim=1)
            pattern_PE = pattern.ndata['distance_PE'].unsqueeze(-1).repeat(1, pattern_emb.size(1))
            pattern_output = pattern_emb + self.PE_alpha * pattern_PE
        else:
        ...
        if self.distance_PE:
            graph_PE = graph.ndata['distance_PE'].unsqueeze(-1).repeat(1, graph_emb.size(1))
            graph_output = graph_emb + self.PE_alpha * graph_PE
            if zero_mask is not None:
                graph_output.masked_fill_(zero_mask, 0.0)
            # graph_PE_embedding = self.PE_linear(graph.ndata['distance_PE'].unsqueeze(-1).to(torch.float))
            # graph_output = torch.cat((graph_emb, graph_PE_embedding), dim=1)
        else:
