...
def compute_recall_precision(predict, exist):
    # predict  = np.where(predict > 0, 1.0, 0.0)
    precision = precision_score(exist, predict)
    recall = recall_score(exist, predict)
    return precision, recall

def compute_confusion_matrix(predict, exist):
    tn, fp, fn, tp = confusion_matrix(exist, predict).ravel()
    return tn, fp, fn, tp

def compute_f1_mcc(predict, exist):
    f1 = f1_score(exist, predict)
    mcc = matthews_corrcoef(exist, predict)
    return f1, mcc
##########################################################
#################### Parsing Functions ###################
##########################################################
...
def load_test_data(graph_dir, pattern_dir, metadata_dir, num_workers=4):
    patterns = read_patterns_from_dir(pattern_dir, num_workers=num_workers)
    graphs = read_graphs_from_dir(graph_dir, num_workers=num_workers)
    meta = read_metadata_from_dir(metadata_dir, num_workers=num_workers)
