...
train_config = {
    ...
    "model" : "RGIN", # CNN, RNN, TXL, RGCN, RGIN, RSIN, GAT
    ...
    "gat_graph_num_layers": 3,
    "gat_pattern_num_layers": 3,
    "gat_hidden_dim": 64,
    "num_heads":1,
...
def evaluate(model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    return mean_bce_loss, evaluate_results
...
if __name__ == "__main__":
    ...
    if all([os.path.exists(os.path.join(train_config["save_data_dir"],
        "%s_%s_dataset.pt" % (
            data_type, "dgl" if train_config["model"] in ["RGCN", "RGIN","GAT"] else "edgeseq"))) for data_type in data_loaders]):

        logger.info("loading data from pt...")
        for data_type in data_loaders:
            if train_config["model"] in ["RGCN", "RGIN","GAT"]:
                ...
                random_data = random.sample(dataset.data, half_length)
                dataset_split = GraphAdjDataset(list())
                dataset_split.data = random_data

                sampler = Sampler(dataset_split, group_by=["graph", "pattern"], batch_size=train_config["batch_size"], shuffle=data_type=="train", drop_last=False)
                data_loader = DataLoader(dataset_split,
